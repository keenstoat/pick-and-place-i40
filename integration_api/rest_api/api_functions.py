...
_DELTA_ROBOT_IP_ADDRESS = "192.168.3.11"
_DELTA_ROBOT_PORT = 502

_robot_ip_address = None
_robot_port = None
...
def init_module():
        global _robot_ip_address, _robot_port
        ...
        _robot_ip_address = data.get("robotIpAddress", _DELTA_ROBOT_IP_ADDRESS)
        _robot_port = int(data.get("robotPort", _DELTA_ROBOT_PORT))
        threading.Thread(target=init, args=[_robot_ip_address, _robot_port]).start()
        response = {
            "data": ""
        }
...
def pick_and_place(x_ini, y_ini, x_end, y_end, object_width, object_height):
    robot = get_robot()
    gripper = Gripper()
...
def get_robot():
    ...
    assert _robot_ip_address and _robot_port, "Robot not initialized. IP Address and port not set."
    robot = DeltaRobot(_robot_ip_address, port=_robot_port)
    assert robot.is_connected, f"DeltaRobot could not connect to {_robot_ip_address} on port {_robot_port}"
    return robot
